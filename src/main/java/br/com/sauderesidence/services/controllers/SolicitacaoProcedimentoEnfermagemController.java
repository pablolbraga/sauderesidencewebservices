...
public class SolicitacaoProcedimentoEnfermagemController {
	
	public boolean inserirSolicitacao(SolicitacaoProcedimentoEnfermagemModel sol) throws SQLException {
		...
		PacienteModel pacienteModel = pacienteController.buscarPacientePorAdmissao(sol.getIdadmission());
			...
			textoSolicitacao = ""				
					...
					+ "<br />Tipo da Solicitação: " + tipoAlteracao + " (PROCEDIMENTO DE ENFERMAGEM)"
					+ "<br />Procedimento: " + sol.getProcedimento()
				...
				+ "<html>"
					...
					+ "<body>"
						+ "<table style='width=100%'>"
							+ "<tr>"
								+ "<td colspan='2' style='width=100%' class='titulo_gg'>Solicitar Alteração de Procedimento de Enfermagem</td>"
							+ "</tr>"
							+ "<tr>"
								+ "<td style='width=50%' class='cor1 fonte2'>Profissional Solicitante:</td>"
								+ "<td style='width=50%' class='cor2 label1'>").append(sol.getProcedimento()).append("</td>"
							+ "</tr>"
							+ "<tr>"
		...
		for (int i = 0; i < emails.size(); i++) {
			Uteis.enviarEmail(emails.get(i).getEmail(), "ALTERAÇÃO DE PRESCRIÇÃO DE ENFERMAGEM", html.toString());
		}
